...
//
//       // If we have a local image path, load it
...
//                 lat: double.parse(_latController.text),
//                 lon: double.parse(_lonController.text),
//                 image: _imageFile?.path ?? _entity!.image, // Keep existing image path if not updated
//               );
//
...
//                 lat: double.parse(_latController.text),
//                 lon: double.parse(_lonController.text),
//                 image: _imageFile!.path, // Store local file path for now
//               );
//
...
//         );
//       }
//       // If editing and the image is a local file path
//       else {
//         return ClipRRect(
//           borderRadius: BorderRadius.circular(8),
//           child: Image.file(
...
class _EntityFormScreenState extends State<EntityFormScreen> {
  ...
  //               );
  //             }
  //             throw e; // Re-throw so it's caught by the outer catch block
  //           }
  //         } else {
  //           // Create new entity locally
  //           final newEntity = Entity(
  ...
  // lib/screens/entity_form_screen.dart (partial update for the _saveForm method)
  Future<void> _saveForm() async {
    if (!_formKey.currentState!.validate()) {
      return;
    }
    ...
    try {
      // Try to connect regardless of offline mode indicator
      if (_isEdit) {
        // Update existing entity
        if (_entity != null) {
          try {
        ...
        if (_imageFile != null) {
          try {
            ...
            // Create locally if online creation fails
            final newEntity = Entity(
